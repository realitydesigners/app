...
export function getMainCategoryPositions(count, offset = [0, 0, 0], radius = 5) {
...
export const MainCategory = props => {
   const { title, model, position, isHighlighted, onClick, onPointerOver, onPointerOut, onCategoryHover, onHover, onLeave, selectedMainWorld } = props;
   ...
   return (
      <group onPointerOver={onHover} onPointerOut={onLeave}>
         <ModelWithEffects model={model} className="model" position={position} scale={[0.5, 0.5, 0.5]} onPointerOver={() => title && onPointerOver(title)} onPointerOut={onPointerOut} onClick={() => title && onClick(title, position)} emissiveIntensity={isHighlighted ? 2 : 1} />

         <Text ref={textRef} position={[position[0], position[1], position[2] + 2.5]} color="white" fontSize={0.6} font={'/fonts/monomaniac.ttf'} anchorX="center" anchorY="middle">
            {title}
         </Text>
      </group>
   );
...
export const SubCategory = props => {
   console.log(props);
   const { title, position, model, isHighlighted, onClick, onPointerOver, onPointerOut } = props;
   ...
   return (
      <group>
         ...
         <Text ref={textRef} position={[position[0], position[1], position[2] + 1.5]} color="white" fontSize={0.3} font={'/fonts/monomaniac.ttf'} anchorX="center" anchorY="middle">
            {title}
         </Text>
      </group>
   );
   ...
   return (
      <>
         {subCategories
            ...
            .map((subCat, index) => (
               <SubCategory key={subCat.title} title={subCat.title} model={subCat.model} position={positions[index]} isHighlighted={subCat.isHighlighted} onClick={subCat.onClick} onPointerOver={subCat.onPointerOver} onPointerOut={subCat.onPointerOut} />
            ))}
      </>
   );
};
