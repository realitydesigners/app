    ...
    <div className="fixed bottom-0 right-0 z-10 mb-4 mr-4">
      {isWorldSelectorVisible && (
        <div className="absolute right-0 bottom-full mb-4 w-64 rounded-lg border border-gray-200/20 bg-black p-4 shadow-lg transition-transform hover:scale-105">
          ...
          ></button>
          <h2 className="mb-4 font-mono text-lg font-semibold text-gray-300">
            Categories
          </h2>
                ...
                {expandedMainCategory === category.title &&
                  ...
                  category.subCategories.map((subCategory) => (
                    <button
                      ...
                      onMouseEnter={() =>
                        onSubCategoryHighlight(subCategory.title)
                      }
                      onMouseLeave={() => onSubCategoryHighlight(null)}
                      className={`mt-2 block w-full pl-8 text-left font-mono uppercase ${
                        selectedSubWorld === subCategory.title
                          ? 'text-white'
                          : 'text-gray-500'
                      } hover:text-gray-300`}
                    >
      ...
      <button
        onClick={toggleWorldSelector}
        className="rounded-full bg-white px-4 py-4 text-black shadow hover:bg-gray-400 focus:outline-none"
      ></button>
    </div>
  )
}
